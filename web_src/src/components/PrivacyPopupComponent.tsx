...
interface IProps {
  ...
  approvePolicy: Function;
}
const PrivacyPopupComponent = (props: IProps) => {
  const {
    showFooter,
    showPopup,
    togglePopup,
  ...
  return (
    <div style={{bottom: !showFooter && '-64px', background: showPopup && 'rgba(0,0,0,.6)'}} className={`${showPopup ? 'inset-0' : 'w-full flex items-center justify-center bg-purple-900 h-12 p-2 bottom-0'} sticky transition-all duration-300`}>
      { !showPopup ?
      <div>
        By using this site you adhere to our privacy policy.
        <button onClick={() => togglePopup()}>
          Privacy policy
        </button>
      </div> :
      <div className="bg-black h-full m-12 p-5 flex flex-col justify-between">
        <span className="text-xl uppercase text-center italics font-semibold">
          Privacy Policy
        </span>
        <p className="p-5 leading-loose overflow-y-scroll h-72">
        ...
        <div>
          <button onClick={() => { approvePolicy(); togglePopup() }}
                  className="p-5 w-full text-2xl">
            Agree
          </button>
        </div>
      </div> }
